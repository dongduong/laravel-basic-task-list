...
class PermissionRoleTableSeeder extends Seeder
{
    public function run()
    {
        ...
        $user_permissions = $admin_permissions->filter(function ($permission) {
            return substr($permission->title, 0, 5) != 'user_' && substr($permission->title, 0, 5) != 'role_' && substr($permission->title, 0, 11) != 'permission_' && substr($permission->title, 0, 10) != 'room_type_' && substr($permission->title, 0, 12) != 'room_status_' && substr($permission->title, 0, 12) != 'task_status_';
        });
