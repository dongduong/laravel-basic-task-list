...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.questions-options.update", [$questionsOption->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('correct') ? 'has-error' : '' }}">
                ...
                <input value="1" type="checkbox" id="correct" name="correct" {{ (isset($questionsOption) && $questionsOption->correct) || old('correct', 0) === 1 ? 'checked' : '' }}>
